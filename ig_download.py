...
def process_post(post, profile_dir, session, delay):
    ...
    try:
        ...
        _common.debug_log("从 _node 解析地理位置失败", e)
    ...
    if not location_name:
        try:
            ...
            _common.debug_log("从 post.location 属性解析失败", e)
...
# ── 统一退避重试网络请求封装 ─────────────────────────────────────────────
@_common.retry_with_backoff(attempts=4, delay=3.0, backoff=2.0)
def _get_json_with_retry(ms, url: str, params=None):
...
def _sync_role1_template_dirs(profile_dir: Path):
    try:
        role1_dir = _common.get_role1_template_dir(profile_dir)
        ...
        _common.debug_log("模板角色文件夹同步失败", e)
...
def _resolve_user_id_by_username(ms, username: str) -> str:
    # 优先从公共 Profile HTML 中直接截取以防止 429 API 限制
    try:
        ...
        _common.debug_log("Profile HTML 直截解析用户ID失败", e)
...
def _extract_mobile_items(ms, user_id: str, limit: int):
    ...
    while fetch_all or len(items) < limit:
        ...
        try:
            ...
            _common.debug_log("分页拉取移动端 Feed 异常", e)
            break
...
def _authenticate(args, L) -> bool:
    ...
    if args.sessionid:
        ...
        for _url in [
            ...
            try:
                ...
                resp = _req.get(
                    ...
                    headers={'User-Agent': MOBILE_UA, 'X-IG-App-ID': INSTAGRAM_APP_ID},
                    timeout=10,
                )
                data = resp.json()
        ...
        if uname:
            ...
            print(f"✓ sessionid 已設置，登入用戶: @{uname}\n")
        else:
            print(f"✓ sessionid 已設置（{sid[:12]}…）\n")
...
def _resolve_posts(args, profile, L, total: int | None, use_mobile_fallback: bool):
    """解析出需要下载的帖子列表及最大数量限制。"""
    ...
    if not use_mobile_fallback:
        try:
            if args.start is not None or args.end is not None:
                if total is None:
                    print("❌  當前會話無法讀取總數，暫不支援 --start/--end 範圍模式")
                    print("    請改用 --count N，或切換登入方式後重試")
                    sys.exit(1)
                ...
                print(f"✓  @{profile.username}  {total} 條帖子，準備下載第 {start}–{end} 條（共 {limit} 條）\n")
            else:
                if args.count:
                    limit = min(args.count, total) if total is not None else args.count
                    posts_iter = islice(profile.get_posts(), limit)
                    total_label = f"{total} 條帖子，" if total is not None else ""
                    print(f"✓  @{profile.username}  {total_label}準備下載最新 {limit} 條\n")
                else:
                    if total is None:
                        limit = None
                        posts_iter = profile.get_posts()
                        print(f"✓  @{profile.username}  當前會話無法讀取總數，將無限制嘗試下載全部帖子\n")
                    else:
                        limit = total
                        posts_iter = islice(profile.get_posts(), limit)
                        print(f"✓  @{profile.username}  {total} 條帖子，準備下載最新 {limit} 條\n")
        except Exception as e:
            _common.debug_log("GraphQL 接口受限，准备进入移动端降级", e)
            use_mobile_fallback = True

    if use_mobile_fallback:
        print("    嘗試切換到移動端 API 降級模式…")
        sid = _get_sessionid_from_loader(L) or _clean_sessionid(args.sessionid)
        if not sid:
            print("❌  降級模式需要有效 sessionid（請填寫 Session ID 或使用可讀 Cookie）")
            sys.exit(1)
        if args.start is not None or args.end is not None:
            print("❌  降級模式暫不支援 --start/--end，請改用 --count")
            sys.exit(1)
        try:
            ...
            if mobile_limit <= 0:
                print("ℹ️  降級模式：未指定 --count，嘗試抓取全部可見帖子（可能較慢）")
            ...
            limit = len(posts)
            print(f"✓  降級模式就緒，獲取到 {limit} 條，準備下載\n")
        except Exception as fallback_err:
            print(f"❌  降級模式失敗：{fallback_err}")
            print("    請更換 sessionid 後重試")
            sys.exit(1)

    return posts_iter, limit, session

def _run_download_loop(posts_iter, limit, profile_dir, session, delay) -> tuple[int, int, int, bool]:
    done = skip = fail = 0
    incomplete = False
    i = 0
    ...
    while True:
        ...
        for attempt in range(1, RATE_LIMIT_RETRIES + 1):
            try:
                ...
                if attempt < RATE_LIMIT_RETRIES:
                    ...
                    print(f"\n⏳  被限流（第 {attempt} 次），等待 {total} 秒後重試…")
                    print(f"    已成功下載 {done} 條 | 錯誤: {e}")
                    try:
                        time.sleep(total)
                    except KeyboardInterrupt:
                        print("\n\n⚠️  手動中斷")
                    ...
                    print(f"\n⚠️  獲取帖子列表失敗（已重試 {RATE_LIMIT_RETRIES} 次）: {e}")
                    print(f"    已成功下載 {done} 條，請等待更長時間後重新運行腳本")
                    incomplete = True
        ...
        try:
            result = process_post(post, profile_dir, session, delay)
            if result == 'ok':
                done += 1
        ...
        if limit is None or i < limit:
            ...
            wait_total = delay + jitter
            print(f"  ⏱  等待 {wait_total:.1f}s…", end='\r')
            time.sleep(wait_total)
            
    return done, skip, fail, incomplete

def _print_summary(done, skip, fail, incomplete, resolved: Path) -> None:
    print(f"\n{'─'*48}")
    print(f"  ✅  完成  下載 {done} | 已有 {skip} | 失敗 {fail}")
    print(f"{'─'*48}")
    if incomplete:
        print("  ⚠️  本次未完整跑完目標列表（通常是限流或網絡導致）")
    print(f"\n  📂  文件夾已自動打開: {resolved}")
    print(f"  🔗  {resolved.as_uri()}")
    print(f"\n  📱  在本地相冊中查看:")
    print(f"      1. 用 Chrome / Edge 打開 local-ig.html")
    print(f"      2. 點「選擇文件夾」，選擇上方目錄")
    print()
    _common.open_folder(resolved)

# ── 主入口 ────────────────────────────────────────────────────────────────────
def main() -> int:
    parser = argparse.ArgumentParser(
        description='Instagram 下載器，配合本地相冊查看器 (local-ig.html) 使用',
    ...
    parser.add_argument('--count', type=int, metavar='N',
        help='最多下载 N 条最新帖子（默认全部）；与 --start/--end 范围模式互斥')
    ...
    # 1. 参数互斥校验校验：--count 与 --start/--end 强制互斥
    if args.count is not None and (args.start is not None or args.end is not None):
        parser.error("參數 --count 與 --start/--end 範圍模式互斥，不可同時指定")
        
    # 2. 范围区间校验校验：start 必须 <= end
    if args.start is not None and args.end is not None:
        if args.start > args.end:
            parser.error("開始範圍 --start 必須小於等於截止範圍 --end")
    ...
    try:
        ...
        if args.sessionid or args.cookies_from_browser:
            print(f"⚠️  Web API 獲取失敗 ({msg})，嘗試使用移動端 API 降級模式…")
            use_mobile_fallback = True
        else:
            print(f"❌  獲取失敗: {msg}")
            sys.exit(1)
    ...
    if not use_mobile_fallback:
        try:
            ...
            _common.debug_log("无法获取 mediacount 属性", e)

        if args.check:
            if total is None:
                print("\n  ⚠️  當前會話無法查詢總數")
                print("  → 請改用 --count N 直接下載最新 N 條\n")
                sys.exit(2)
            print(f"\n  ✓  @{profile.username}  共 {total} 條帖子")
            print(f"\n  → 請將 {total} 填入頁面的帖數輸入框，選擇下載範圍\n")
            sys.exit(0)

    # 4. 子逻辑：解包数据迭代器
    posts_iter, limit, session = _resolve_posts(args, profile, L, total, use_mobile_fallback)

    # 5. 子逻辑：执行下载主循环
    done, skip, fail, incomplete = _run_download_loop(posts_iter, limit, profile_dir, session, args.delay)

    # 6. 子逻辑：打印结果概要
    if profile_dir:
        _print_summary(done, skip, fail, incomplete, profile_dir.resolve())
